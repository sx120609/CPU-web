...
private struct WatchSyncBadge: View {
    ...
    static func value(for state: ScheduleDisplayState) -> (label: String, symbol: String, tint: Color)? {
        ...
        case .failed(let failure):
            switch failure {
            case .unsupportedVersion: return ("请更新两端 App", "arrow.down.app", .orange)
            case .invalidData: return ("课表格式错误", "doc.badge.ellipsis", .orange)
            case .storage: return ("缓存读取失败", "externaldrive.badge.exclamationmark", .orange)
            case .sourceUnavailable: return ("课表暂不可用", "wifi.exclamationmark", .orange)
            default: return ("同步失败", "exclamationmark.triangle", .orange)
            }
        }
    }
...
struct WatchScheduleUnavailableView: View {
    ...
    private var title: String {
        ...
        case .failed(.unsupportedVersion): return "需要更新 App"
        case .failed(.invalidData): return "课表格式不受支持"
        case .failed(.storage): return "无法读取课表缓存"
    ...
    private var message: String {
        ...
        default: return "在 iPhone 打开药大拾间的原生课表，数据会自动发送到手表。"
        }
    }
}
