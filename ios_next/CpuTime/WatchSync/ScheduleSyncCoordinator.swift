...
final class ScheduleSyncCoordinator {
    ...
    init(role: Role, repository: CourseRepository, transport: any ScheduleTransport,
        ...
        transport.onClear = { [weak self] in self?.clearReceivedSnapshot() }
    ...
    /// Clears account-scoped course data before publishing a signed-out status.
    ...
    func clearForAccountChange() {
        do {
            ...
            lastQueuedAt = nil
            lastSyncedAt = nil
            defaults?.removeObject(forKey: "watchScheduleLastAcknowledged")
            error = .loginRequired
            sendSourceStatus()
            onSnapshotChange?()
            onChange?()
    ...
    private func clearReceivedSnapshot() {
        guard role.receivesSnapshots else { return }
        do {
            try repository.clear()
            lastSentData = nil
            lastSyncedAt = nil
