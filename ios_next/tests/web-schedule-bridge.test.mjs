...
test('2025-2026-2 collapses explicit source ids when one row is only a week subset', async () => {
  ...
  ctx.api.schedule = async () => ({ parsed: {
    ...sample(),
    currentSemester: '2025-2026-2',
    cells: [{ day: 3, bigSlot: 2, courses: [
      { name: '实验课', teacher: '张老师', location: '实验楼201', weeks: '1-8周', weekList: [1,2,3,4,5,6,7,8], sourceKey: 'jwxt:data-jxbid:row-a', startSlot: 3, endSlot: 4 },
      { name: '实验课', teacher: '张', location: '201', weeks: '2、4、6、8周', weekList: [2,4,6,8], sourceKey: 'jwxt:data-jxbid:row-b', startSlot: 3, endSlot: 4 },
    ] }],
  } });
  const result = await ctx.window.CPUTimeNativeScheduleFetch('2025-2026-2', '1');
  const courses = result.data.cells.flatMap(cell => cell.day === 3 ? cell.courses : []);
