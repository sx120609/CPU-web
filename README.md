...
> - 二手功能是论坛信息交流板块；本站不提供站内下单、支付、担保、退款或结算。
...
| 热榜 / 最新 / 二手交流 / 课程点评 | `/forum/hot` `/forum/latest` `/market` `/coursereview` | 浏览按社区权限控制，发布需站内登录 | 受 `forum` / `market` / `coursereview` 功能开关与社区权限控制 |
...
- 论坛社区：支持普通讨论、提问、树洞、课程点评、匿名发帖、点赞、消息通知与站内私聊；私聊记录持久化保存，发起方在对方首次回复前最多发送两条。
- 二手交流：作为论坛板块提供闲置、求购和经验帖，复用论坛的发帖、回复、审核与管理能力，不涉及站内交易。
...
| `npm run build` | 构建 Server、Web 与 VoiceHub |
| `npm run verify:push` | 每次远程推送前必须通过的三项目完整生产构建 |
...
| `MARKET_ENCRYPTION_SECRET` | 从 `JWT_SECRET` 派生 | 仅用于兼容保留的旧商城数据；当前二手论坛不读取收款资料或电子交付内容 |
...
每次远程推送前必须在最终提交上运行 `npm run verify:push`，完整构建 Server、Web 与 VoiceHub；详细门禁与完成判定见 [`docs/production-requirements.md`](docs/production-requirements.md)。`main` 的每次推送（包括仅修改文档或工作流）都会触发 `Linux deployment artifact` GitHub Actions。工作流在 Ubuntu/Node 24 上构建 `server/dist`、`web/dist` 与 `voicehub/.output`，生成绑定完整提交 SHA 的清单和三个 SHA-256 校验值，同时上传短期 Actions 审计制品、强制更新仅含当前制品的 `deploy-artifacts` Git 传输分支，并更新同名预发布资产用于审计和手动取回。`./deploy.sh update` 默认使用 `DEPLOY_BUILD_MODE=auto`：只经仓库当前 `origin`（可为已配置的 GitHub 镜像）取得制品，等待并验证它与当前 `HEAD` 完全匹配后再暂存、原子发布和健康检查；CI 制品不可用时才回退到受保护的本机编译。只有显式设置 `DEPLOY_ARTIFACT_URL` 时才会启用额外的 HTTP 下载备用源，默认不会绕过 `origin` 直连 GitHub。设置 `DEPLOY_BUILD_MODE=ci` 可禁止生产机编译并在制品缺失时失败关闭，设置为 `local` 可强制使用资源受限的本机编译。`./deploy.sh artifact-check [完整提交 SHA]` 只下载、校验并缓存制品，不发布文件。
...
- 二手交流只承载公开帖子与回复，不承诺交易真实性，也不提供资金托管或纠纷结算；用户应自行核验信息并优先在安全的校内公共场所沟通。

## 开源与非商业使用说明

药大拾间由学生自主开发和维护，仅供个人学习、技术研究、校园公益及非商业社区使用。**严禁未经项目维护方书面授权，将药大拾间品牌、官方构建、在线服务、生产资源或运营成果用于收费服务、商业推广、转售、付费部署及其他营利活动。**

站点名称、Logo、域名、官方安装包、生产配置、密钥、用户数据、学校标识、第三方素材和部署数据不随代码许可授权。不得以“药大拾间官方”“学校官方”或其他可能造成混淆的名义运营衍生项目。

仓库中可识别的开源代码与第三方组件仍按各自许可证执行；本说明不会改变 OCS、Electron 等第三方项目的既有许可，也不追溯撤销已经按既有许可证取得的权利。

## License

本项目建议采用 `AGPL-3.0-or-later` 作为代码开源协议；正式发布时请以仓库顶层 `LICENSE` 文件为准。

选择 AGPL 的原因是本项目主要以 Web 服务形式运行。社区可以自由学习、部署、修改和贡献代码；如果修改后的版本通过网络向用户提供服务，也应向相应用户开放对应源码。药大拾间品牌、官方构建、在线服务和生产资源不包含在该代码许可中，严禁未经授权用于商业用途。
