...
- 课表支持周 / 日双视图、左右滑动翻周、本地缓存与相邻周预热，并提供 PWA 清单与 Service Worker；移动端客户端和添加到桌面的课表页在成功打开一次后可离线重开查看。
...
- `sw.js` 会缓存 `/schedule` 页面壳、图标和当前已加载的静态资源；在线成功打开一次后，断网重开仍能进入课表页。若完整前端资源暂时拉不起来，会自动退回到内置的“离线课表页”，直接读取本地缓存展示最近一次课表。
- 课表数据继续走 localStorage 缓存（学期 + 周次为 key，12 小时 TTL）；断网时优先展示最近一次成功读取的内容，并保留相邻周预热能力。
