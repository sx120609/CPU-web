  ...
  <main class="shared-schedule-page">
    ...
    <section v-else-if="share" class="schedule-sheet">
      ...
      <div class="schedule-grid">
        <div class="grid-head corner">节次</div>
        <div v-for="day in 7" :key="day" class="grid-head" :class="{ today: dayDates[day - 1] === today }">
          <b>{{ dayNames[day - 1] }}</b><small>{{ dayDates[day - 1]?.slice(5) || "--" }}</small>
        </div>
        <template v-for="slot in slots" :key="`slot-${slot.no}`">
          <div class="slot-axis"><b>{{ slot.no }}</b><small>{{ slot.start }}</small><small>{{ slot.end }}</small></div>
          <div v-for="day in 7" :key="`${slot.no}-${day}`" class="grid-cell" :class="{ today: dayDates[day - 1] === today }" />
        </template>
...
import type { ScheduleResult } from "@/views/schedule/types";
...
<style scoped>
.shared-schedule-page{min-height:100vh;padding:28px max(16px,4vw) 48px;background:#f6f8fb;color:#172033}.shared-head{display:flex;align-items:flex-end;justify-content:space-between;gap:18px;max-width:1180px;margin:0 auto 20px}.eyebrow{margin:0 0 5px;color:#168776;font-size:12px;font-weight:700;letter-spacing:.04em}.shared-head h1{margin:0;font-size:28px}.shared-head p:last-child{margin:7px 0 0;color:#667085}.head-actions{display:flex;align-items:center;gap:8px}.schedule-sheet{max-width:1180px;margin:0 auto;padding:16px;border:1px solid #dbe4ee;border-radius:12px;background:#fff;box-shadow:0 10px 28px rgba(24,34,51,.07)}.week-title{display:flex;align-items:center;gap:14px;margin:0 0 12px;color:#667085;font-size:13px}.week-title strong{color:#172033;font-size:16px}.schedule-grid{position:relative;display:grid;grid-template-columns:54px repeat(7,minmax(72px,1fr));grid-template-rows:40px repeat(11,minmax(52px,1fr));overflow:auto;border-top:1px solid #dbe4ee;border-left:1px solid #dbe4ee}.grid-head,.slot-axis,.grid-cell{min-width:0;border-right:1px solid #dbe4ee;border-bottom:1px solid #dbe4ee}.grid-head{display:flex;align-items:center;justify-content:center;gap:3px;flex-direction:column;background:#f8fafc;font-size:12px}.grid-head small,.slot-axis small{color:#8a94a6;font-size:10px}.grid-head.today{background:#e9f8f3}.slot-axis{z-index:1;display:flex;align-items:center;justify-content:center;flex-direction:column;gap:2px;background:#f8fafc;color:#667085;font-size:11px}.grid-cell.today{background:#f3fbf8}.course-block{z-index:2;display:flex;min-width:0;margin:3px;padding:6px;border-radius:7px;background:#d9f4ec;color:#145b50;box-shadow:inset 0 0 0 1px rgba(22,135,118,.15);flex-direction:column;overflow:hidden}.course-block strong,.course-block span,.course-block em{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.course-block strong{font-size:12px}.course-block span,.course-block em{margin-top:3px;font-size:10px;font-style:normal}.course-block em{color:#53857c}.loading-state{max-width:720px;margin:80px auto}@media(max-width:680px){.shared-head{align-items:flex-start;flex-direction:column}.head-actions{width:100%}.head-actions .el-select{flex:1}.shared-head h1{font-size:24px}.schedule-sheet{padding:8px}.week-title{flex-wrap:wrap}.schedule-grid{grid-template-columns:46px repeat(7,74px)}}
</style>
