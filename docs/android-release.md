...
- 仓库的 `web/public/downloads/` 只保留发布清单当前指定的 APK，供 Linux 工作流校验签名、包名和版本；历史版本保留在企业盘，不再提交到仓库。
...
6. 选择性提交发布清单及签名包，并在同一提交中删除 `web/public/downloads/` 里上一版 APK，使该目录只剩当前版本。前后端共用这份清单；Linux 工作流会在编译网页之前再次验证企业盘和安装包，并将验证记录上传为制品。
