...
已经批准的首次迁移可显式设置 `DEPLOY_AGENT_GATEWAY_BOOTSTRAP=1`：仅对尚无独立网关的旧版 active 状态有效。该模式先启动并验证候选网站，保留旧版公共 API，然后最多等待 5 分钟让两台 Agent 在网关就绪；全部基线检查仍须在实际 API 切换前通过。此标志不会继承到 PM2 环境，不能用于后续部署。看到候选就绪日志后才执行批准的连接重建；超时不切换 API。若已修改 Agent 路由但迁移失败，须用迁移前备份恢复路由并让 Agent 返回旧服务，不能仅恢复网站端口。
