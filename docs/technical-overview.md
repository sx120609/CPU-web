...
- 论坛、教务、商城、文件收集、拾间 AI、药苑之声等模块如何协作；
...
| 身份 | 用途 | 是否等同 |
|---|---|---|
| 药大拾间站内账号 | 论坛、通知、商城、AI 额度、工具权限和个性化设置 | 否 |
...
同步版块通常以只读为主，避免用户误以为在药大拾间回复就一定会回写到外部平台。
...
商品支持实体和数字类型，并经历草稿、审核中、上架、预留、售出、下架等状态。标题、说明和图片进入商城自己的审核流程。
...
卖家接受报价后生成订单。订单在限定时间内等待支付：

1. 服务端创建支付单并计算金额；
2. 页面跳转到配置的支付渠道；
3. 支付渠道回调主站；
4. 主站验证签名、金额、订单号和回调状态；
5. 支付日志写入数据库；
6. 订单进入交付流程。

前端显示“支付成功”不能单独作为到账依据，必须以服务端验证过的回调为准。

### 11.4 交付、退款和结算

- 实体商品支持线下见面或约定交付及双方确认；
- 数字商品在支付后按规则发放，敏感交付内容加密保存；
- 订单支持取消、退款、争议和评价；
- 卖家收款资料加密保存，对页面只返回脱敏信息；
- 结算区分可结算、暂扣和已结算金额；
- 平台费率、分类、支付和结算规则由后台配置。

---

## 12. 赞助、额度和账本

赞助模块与商城共用部分支付基础能力，但业务目的不同：
...
论坛回复、点赞、商城状态、文件任务、失物认领、工具提醒和药苑之声事件都可以生成站内通知。用户可以查看已读/未读状态，并调整部分消息设置。
...
内容审核服务处理论坛、商城、失物招领等公开内容。它关注违规风险和人工复核建议，不负责聊天，也不负责解题。
...
| 校园网 Agent 全部离线 | 校内教务请求暂不可用 | 不影响论坛、公告、商城等站内功能 |
