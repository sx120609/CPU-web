...
2. **多平台网课窗口** —— 独立的受控 Chromium 会话，支持超星、智慧树、智慧职教、职教云、中国大学 MOOC 与雨课堂
...
| 白名单 | 作用 | 当前成员 |
|---|---|---|
| `navigableHosts` | 允许在应用窗口内打开 | 主站、六个正式网课平台的明确域名，以及只供登录跳转的 `cpu.edu.cn` |
| `injectableHosts` | 允许注入用户脚本、允许持有脚本特权桥 | 超星、智慧树、智慧职教/职教云、中国大学 MOOC、雨课堂的审核域名；没有教育网泛域名 |
...
两个 preload 互不可见：主站窗口挂 `site-preload.ts`（`window.CPUDesktop`），网课窗口挂 `learning-preload.ts`（`window.cpuDesktopBridge`）。主站拿不到脚本代理，网课平台也拿不到桌面端外壳能力。
...
AI 请求体按服务端答题接口接受的字段做严格白名单，其余字段丢弃。客户端每次打开网课平台、每次发起答题前都会读取服务端访问策略：临时开放时走 `/api/site/learning-assistant/responses`，不扣账号额度；恢复限制后走 `/api/oauth/v1/responses`。access token 始终留在主进程，不下发渲染进程。
...
- 多平台识别与任务适配基于 [ocsjs/ocsjs](https://github.com/ocsjs/ocsjs) 4.15.3（MIT）整合；外部题库权限已移除，固定来源与修改记录见第三方声明。

客户端工具页的“关于”区域会同步展示当前版本、上述贡献来源、开源许可和非学校官方声明。

> **非商业使用声明：** 药大拾间官方客户端仅供学习研究与校园公益使用，严禁未经授权的商业用途。药大拾间名称、图标、官方构建与在线服务不随代码许可证授权；第三方组件仍按各自许可证执行。

## 药大拾间·多平台网课助手

工具页的“打开网课平台”会弹出统一面板，目前接入 OCS 4.15.3 正式注册的六个平台：

| 平台 | 当前接入能力 |
|---|---|
| 超星学习通 | 沿用药大拾间专用引擎：视频/音频、章节任务、测验、作业、考试、暂存与截图搜题 |
| 知到智慧树 | 共享课/校内课的视频学习与作业、考试识别 |
| 智慧职教 / MOOC | 课程视频、任务与测验识别 |
| 职教云 | 课程学习、视频与作业识别 |
| 中国大学 MOOC | 课程视频与章节测验识别 |
| 雨课堂 | 视频、作业与考试页面识别 |

超星已有专用引擎命中时不会再启动 OCS 引擎，避免同一页面重复点击、重复答题和重复消耗点数。
其余平台由 `assets/userscripts/multiplatform.js` 运行。它保留 OCS 的平台识别、视频学习、任务调度、
作业/考试识别与答案填写能力，但移除了所有外部题库联网权限；统一通过宿主虚拟地址调用药大拾间
独立答题 AI。关闭“自动提交”时仍调用平台自身的保存/暂存动作，不替用户正式交卷。

OCS 引擎所需的 GM 存储、标签状态、通知与跨域请求由 `main.ts` 的受限桥提供。外部题库配置即使被
上游界面写入，也无法绕过 `@connect` 和主进程地址校验。当前多平台引擎随客户端 0.3.0 发布；涉及
新增平台域名或权限时，Windows 与 macOS 必须同时提升客户端版本，不能通过云端脚本静默扩权。
来源、固定上游 commit、生成方式与许可见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)。
...
- 缺 `minAccuracy` → 旧版脚本的答案完整性保护会失效

**夹回危险取值。** 脚本对这些值零校验：间隔填 0、最小值大于最大值都会让它秒答秒交。这些只能在客户端拦。

界面只暴露用户真正需要调整的自动化开关、每题间隔与 AI 模式。切章和提交前等待使用固定安全值；所谓“正确率”其实只是答案覆盖率，因此不再作为设置暴露。脚本自带表单里那个「自动答题」开关全文没有任何读取点，是装饰品，没有放进来。
...
从客户端 0.2.0 起，学习通标签是否需要药大拾间账号、AI 作答是否扣每日额度，不再写死在安装包里。
后台「AI 额度管理 → 学习通助手访问策略」可以在两种模式间切换：

- `guest-unlimited`：安装客户端即可使用，免登录且不扣每日额度与 AI 点数；
- `account-quota`：必须完成 OAuth 登录，按账号每日额度和 AI 点数使用。

客户端只短暂缓存公开策略用于界面展示；每次免登录答题仍由服务端再次检查当前策略。后台切回
`account-quota` 后，旧客户端不能靠本地缓存延长临时权限，也无需为了恢复限制重新发安装包。
“不限次数”只表示不扣业务额度，服务端仍保留按 IP 的瞬时限流，防止脚本异常循环拖垮服务。

## 多平台「记住密码」

平台选择面板为六个平台分别提供默认关闭的“记住此平台账号密码”。每个平台独立开关、独立加密文件；
超星继续沿用旧版 `chaoxing-login.bin`，已有用户升级后不用重新登录，其余平台使用
`learning-login-<platform>.bin`。全部通过 Electron `safeStorage` 加密，代码见
[chaoxing-credentials.ts](electron/chaoxing-credentials.ts)。

信任边界与校园网凭据一致，只多一件事：密码必须回到平台登录页的输入框里，否则无法自动填充。
为此收窄成：

- 平台身份在创建标签时由主进程绑定，不接受网页自报；新窗口继承原标签的平台身份。
- 明文凭据只在发起 frame 属于该平台审核域名、协议为 HTTPS 且页面实际出现密码框时下发。
- 应用外壳只能拿到打码后的账号与 `hasCredential`，拿不到密码；不同平台之间也不能互读。
- 登录弹窗晚于页面加载出现时，preload 会短时观察 DOM，发现账号密码表单后再填充并捕获登录动作。
- 关掉某个平台的开关会立即删除该平台凭据。短信验证码与二维码没有密码，不会被保存。
...
- 答案来自独立的答题 AI 通道，只发送当前题干、选项与题目图片，不注入站内知识库、历史会话或用户资料；是否免登录、是否消耗每日额度由服务端访问策略实时决定，模型由站点后台设定，客户端不参与选择。
