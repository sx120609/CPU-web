...
AI 请求体按服务端答题接口接受的字段做严格白名单，其余字段丢弃。客户端每次打开学习通、每次发起答题前都会读取服务端访问策略：临时开放时走 `/api/site/learning-assistant/responses`，不扣账号额度；恢复限制后走 `/api/oauth/v1/responses`。access token 始终留在主进程，不下发渲染进程。
