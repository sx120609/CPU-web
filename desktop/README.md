...
| 改动 | 生效时机 |
|---|---|
| AI 开关与模型、自定义题库三项 | 立即生效（脚本每道题现读现用） |
| 四个自动化开关、五个间隔、正确率阈值 | **要重新打开学习平台窗口** |

设置界面会在改到后者时提示。

### 运行状态回传

脚本自己的面板只保留 20 条日志，且关掉面板就什么都看不见。客户端在脚本的日志与状态出口各加了一行上报（`GM_cpuReport`），把消息转发到主进程存一份，「PC 小工具」面板里能直接看到当前进度与最近日志，不用去学习平台窗口里找那个悬浮球。
