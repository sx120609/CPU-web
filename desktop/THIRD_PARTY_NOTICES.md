...
**署名与上游版本可追溯**：元数据头中的 `@author`（shushoujiu）与 `@namespace`（askAuto）保持不变；上游基线为 2.1.6。本项目因品牌、独立 AI、安全边界、章节/作业/考试引导、云端热更新及服务端动态访问策略适配，将发行版本提升为 2.2.13，原脚本地址见上表。
...
## 2. 多平台网课适配 — OCS 4.15.3（MIT）

| 项目 | 内容 |
|---|---|
| 文件 | `assets/userscripts/multiplatform.js` |
| 上游 | [ocsjs/ocsjs](https://github.com/ocsjs/ocsjs) |
| 版本 | 4.15.3，commit `890686a5e54f9a6d52d1169bae9ea5971e0863c7` |
| 原作者 | enncy 及 OCS 贡献者 |
| 许可 | MIT；完整文本见 `assets/vendor/ocsjs-LICENSE.txt` |

本客户端整合 OCS 当前正式注册的超星、智慧树、智慧职教、职教云、中国大学 MOOC、雨课堂项目。
超星页面优先使用药大拾间长期维护的专用引擎，避免两套自动化在同一页重复点击；其余平台使用
OCS 多平台引擎。具体生成来源与复现命令见 `assets/vendor/ocsjs-PROVENANCE.md`。

为适配本客户端所做的变更：

- 将脚本名称、主页与更新入口改为药大拾间品牌；保留 OCS 作者、版本、来源与 MIT 许可。
- 移除 `*.edu.cn`、`*.org.cn` 泛域名匹配，只保留上游正式平台的明确域名并限定 HTTPS。
- 删除全部外部题库 `@connect`，仅保留宿主内部虚拟地址 `desktop.localhost`；题目统一通过主进程
  调用药大拾间独立答题 AI，不向 OCS 题库、第三方题库或平台外站点发送。
- 由宿主补齐 OCS 依赖的 GM 存储、标签状态、通知和跨域请求兼容层；客户端答题间隔、
  自动提交选择与 AI 难度会映射到 OCS 全局配置。
- 移除“禁止最小化/切屏”的浏览器环境警告；Electron 网课视图已关闭后台节流，最小化、切换标签
  或切换桌面不会由客户端主动暂停媒体和任务计时。
- 关闭自动提交时，答案仍会执行平台提供的“保存/暂存”，不会替用户正式交卷。
- 平台权限随 Windows/macOS 客户端版本审核发布，不能由云端正文静默扩大。

---

## 3. 校园网认证模块 — 移植自 cpu_net（GPL-3.0）
