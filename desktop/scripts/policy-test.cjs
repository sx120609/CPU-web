...
check("安全微伴脚本品牌和匹配范围已写入内置脚本", () => {
  ...
  assert.match(source, /const ANSWER_BANK_RAW\s*=\s*'https:\/\/github\.com\/hangone\/WeBan\/raw\/refs\/heads\/main\/answer\/answer\.json'/);
  assert.match(source, /const ANSWER_BANK_MIRRORS\s*=\s*\[/);
  assert.match(source, /gh-proxy\.com\/'\s*\+\s*ANSWER_BANK_RAW/);
  assert.match(source, /mirror\.ghproxy\.com\/'\s*\+\s*ANSWER_BANK_RAW/);
  assert.match(source, /ghproxy\.net\/'\s*\+\s*ANSWER_BANK_RAW/);
  assert.match(source, /raw\.gitmirror\.com\/hangone\/WeBan\/refs\/heads\/main\/answer\/answer\.json/);
...
check("学习通助手品牌、版本和进入课程引导已写入内置回退脚本", () => {
  ...
  assert.match(source, /^\/\/ @version\s+2\.2\.15$/m);
