...
async function main() {
  ...
  assert.deepEqual(multiplatformIdentity, { name: "药大拾间·全平台网课助手", version: "4.15.6" });
  ...
  try {
    ...
    const olderSource = multiplatformSource.replace("// @version      4.15.6", "// @version      4.15.5");
    assert.deepEqual(
    ...
    assert.equal(cached?.manifest.version, "4.15.6");
    ...
    assert.equal(protectedFromDowngrade.manifest.version, "4.15.6");
