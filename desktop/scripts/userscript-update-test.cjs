...
async function main() {
  ...
  assert.deepEqual(identity, { name: "药大拾间·学习通助手", version: "2.2.7" });
  ...
  try {
    ...
    const olderSource = source.replace("// @version      2.2.7", "// @version      2.1.9");
    ...
    assert.equal(cached?.manifest.version, "2.2.7");
