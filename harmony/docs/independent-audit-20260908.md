...
## 启动与返回链路补充（2026-09-09）

普通冷启动默认进入原生课表，后台网页用于恢复共享会话。可信主文档在首帧前隐藏重复的网页启动层；原生启动等待在页面发出内容就绪信号后只结束一次，后续路由切换不再重复展示。超时保留重试状态。

鸿蒙桥统一原生返回、系统返回、router.back 和页面的语义返回按钮，保存当前导航链并裁剪已返回的路径，避免论坛列表和帖子往返形成循环；直接进入子页面时按路由家族回到上级。底栏切换重置当前导航链。编辑器内部返回与弹窗操作不由该页面级处理器接管。

离开课表时，底栏持续显示到目的页面绘制完成；课表淡出后切为 Hidden，并立即禁用其触摸，返回课表会取消延迟隐藏。保持 Web 实例存活，防止切页重建。95 项自动回归通过，涵盖旧导航完成回调、快速切回课表、淡出后退场、论坛返回循环、查询参数及直接进入 20 类路由的上级回退。
