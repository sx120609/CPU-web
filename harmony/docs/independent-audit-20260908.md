...
## 原生导航、卡片排版与图片预览（2026-09-09）

本轮按用户提供的 iOS 截图调整课表比例，保留完整 11 节时间轴，缩小周次标题并为课程、教室文本设置字号适应。网页公共顶栏改为 ArkUI 组件，保留 Logo、消息与更多入口；底栏改为 48vp 原生胶囊，选中项具有独立背景。投稿按钮使用同一底部避让距离。图片预览工具栏避开状态栏，支持下滑关闭；通用网页相册在鸿蒙优先使用现有原生预览桥，自定义回调保留网页处理。

桌面卡片采用浅深模式彩色底块，2×2 卡片仅展开第一门课程并显示下一节时间，修复长名称换行挤压的问题。同名课程保持同色，理论课与实验课的既有哈希碰撞已修正。大卡片根据可用高度截取课程并显示剩余数量。

手动课表刷新合并重复请求、失败保留缓存，已知学期的校历与个人修改并行读取；鸿蒙直接刷新选中周，取消该路径多余的强制会话探测及旧服务端不支持的全周探测。实测后端课表请求仍有约 2.5 秒延迟，不能据此声称整体刷新时间固定变快。iOS 默认调用路径不启用该优化选项。

本机证据位于 `output/harmony-independent-qa-20260908/`，真实账号截图不提交到 Git：

- `bar-fab-final.jpeg`：Logo、最终胶囊底栏与修正后的投稿按钮距离。
- `schedule-check.jpeg`、`dark-final.jpeg`：课程教室编号、末节结束时间及浅深课表；前一张仍含关闭中的详情面板，不作为底栏最终截图。
- `small-real.jpeg`：实际桌面小号和中号卡片，长课程名完整显示，粉／蓝课程区分清楚。
- `widget-small-fixed.jpeg`：实际桌面两日大卡片；`widget-dark-preview.jpeg`：深色小号卡片共用组件预览。
- `image-safe-area.jpeg`：原生关闭、计数、保存按钮与底部标题完整显示。后续将预览状态的系统栏颜色与黑色背景统一。
- `final-tests.log`：86 项回归测试通过；`web-typecheck.log`：Web 类型检查通过；`final-build.log`：最终 ArkTS、资源和无签名调试 HAP 打包通过。

页面入场恢复 220ms 淡入，原生课表切换添加淡入和轻移，并继续等待网页目的页面完成绘制后交接。截图只验证布局和静止状态，不作为动画流畅度或无闪烁的证明。按用户要求，本轮后续验证仅使用 HDC 和 ArkWeb 调试接口，不使用 Computer Use。没有执行生产部署，未验证已签名真机发布包或真实锁屏宿主。
