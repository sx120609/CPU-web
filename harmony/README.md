...
- ArkUI 原生顶栏保留应用 Logo、刷新、消息和更多入口，同步网页登录、未读数和路由标题；有独立导航的页面保留自己的顶栏。原生底栏高度为 56vp，保留完整系统底部安全区并至少预留 28vp，投稿按钮和页面留白使用同一避让距离，网页弹层及帖子详情收起原生底栏。网页入场恢复 220ms 淡入，课表切换使用原生淡入和轻移，保持同一个 ArkWeb 实例。
