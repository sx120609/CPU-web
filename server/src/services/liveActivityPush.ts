...
export function planPayload(event: string, raw: string, timestamp: number) {
  ...
  if (event === "start") {
    ...
    aps.alert = { title: "课程提醒", body: "课表实况活动已开始" };
  }
...
export async function registerLiveActivityDevice(input: {
  ...
  const provider = await getApnsConfig();
  if (provider.configured && bundleID !== provider.bundleID) throw new Error("Bundle ID 与 CPU APNs 配置不一致");
  const startToken = input.startToken ? parseToken(input.startToken, "startToken") : "";
  let existing = input.deviceID
  ...
  if (tokenHash && !existing) {
    const tokenDevice = await db.liveActivityDevice.findUnique({ where: { startTokenHash: tokenHash } });
    if (tokenDevice?.userId === input.userId) existing = tokenDevice;
    else if (tokenDevice) {
      // Possession of the current APNs token identifies the physical device.
      // Drop the previous account's plan before transferring that device.
      await db.liveActivityDevice.deleteMany({ where: { id: tokenDevice.id, startTokenHash: tokenHash } });
    }
  }
...
async function ensureBroadcastEvents(now: number) {
  ...
  const today = dateKeyAt(new Date(now * 1000));
  const term = terms.find((item) => today >= item.semesterStartMonday && today < addDays(new Date(`${item.semesterStartMonday}T00:00:00Z`), item.weekCount * 7).toISOString().slice(0, 10));
  if (!term) return;
    ...
    for (let offset = -1; offset < 9; offset += 1) {
      ...
      for (const period of term.periods) {
        for (const [phase, clock] of [["started", period.start], ["ended", period.end]] as const) {
          ...
          rows.push({ channelID, environment, bundleID: config.bundleID, eventID, fireAt: instant, expiresAt: new Date(instant.getTime() + 60_000), event: payload.aps.event, payload: JSON.stringify(payload) });
        }
      }
    }
  }
...
export async function tickPlannedLiveActivities() {
...
async function tickLiveActivities() {
  ...
  for (const row of rows) {
    ...
    if (end !== null && end <= now) {
      const pushed = await pushRegistration(row, { aps: { timestamp: Math.floor(now), event: "end", "content-state": state, "dismissal-date": Math.floor(now) } });
...
export function startLiveActivityPushScheduler() {
  ...
  const tick = async () => {
    ...
    try {
      ...
      delay = (await getApnsConfig()).tickSeconds * 1000;
