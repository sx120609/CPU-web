...
const CAMPUS_ASSISTANT_KNOWLEDGE: CampusAssistantKnowledge[] = [
  ...
  },
  {
    ...
    fact: "刷课与安全教育说明：药大拾间客户端的学习通助手目前只支持学习通，其中的解题功能受题型、页面和上游服务影响，不保证每次都能正常使用。江苏省大学生安全教育考试可以直接在 QQBot 内完成；安全微伴可使用 QQ 用户群群文件中的程序，安全微伴账号和密码均为学号。具体使用方式和最新说明建议加入 QQ 用户群（704825850）了解详情。",
