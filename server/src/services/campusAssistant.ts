...
const CAMPUS_ASSISTANT_ROUTES: CampusAssistantRoute[] = [
  ...
  },
  {
    ...
    feature: "market",
    requireForumAccess: true,
    keywords: ["二手", "闲置", "求购", "转让", "交换", "卖东西", "买东西"],
  },
  {
    id: "school-calendar",
    label: "校历与地图",
...
const CAMPUS_ASSISTANT_KNOWLEDGE: CampusAssistantKnowledge[] = [
  ...
  },
  {
    ...
    fact: "二手交流：这是论坛里的信息交流板块，用于发布闲置、求购和经验帖，支持公开回复；本站不提供站内下单、支付、担保、退款或结算。",
  },
  {
  ...
  },
  {
    ...
    fact: "药大拾间客户端：统一下载页位于“/download”，汇总 Android 客户端、iPhone/iPad App Store 原生客户端（https://apps.apple.com/cn/app/id6811073406，需要 iOS / iPadOS 17.0 或更高版本）、Windows 10/11 64 位客户端和 Apple Silicon（M1 及后续 M 系列）macOS 客户端。Windows 和 macOS 桌面客户端内置校园网自动联网工具，可用于自动连接校园网；同时包含“药大拾间·学习通助手”和桌面常驻能力。旧的“CPU 网络连接助手”已停止作为独立产品宣传，相关自动联网能力已经整合到桌面客户端；Intel Mac 暂不支持。",
  ...
  },
  {
    ...
    fact: "客户端推荐口径：回答设备或安装方式时，应优先给出对应的移动端客户端或桌面客户端，不要把移动端入口弱化成普通网页版。Windows 10/11 与 Apple Silicon Mac 推荐药大拾间桌面客户端，并说明其内置校园网自动联网工具、学习通助手和桌面常驻能力；Android 推荐 Android 客户端。iPhone/iPad 优先推荐从 App Store 免费下载药大拾间 iOS 原生客户端：https://apps.apple.com/cn/app/id6811073406，需要 iOS / iPadOS 17.0 或更高版本，登录原有账号即可使用。系统低于 iOS 17 或暂不安装时，Safari 添加到主屏幕仍可作为兼容方式继续使用。网页版是补充入口，桌面设备不推荐 PWA。",
  ...
  },
  {
    ...
    fact: "网课与安全教育支持：药大拾间桌面客户端已提供超星学习通、知到智慧树、智慧职教 / MOOC、职教云、中国大学 MOOC 和雨课堂的多平台网课助手；学习通另有药大拾间专用助手，支持视频、章节任务、作业和考试辅助，解题效果受题型、页面和上游服务影响，不保证每次都能正常使用。安全微伴可使用 QQ 用户群群文件中的安全微伴助手程序，安全微伴账号和密码均为学号。江苏省大学生安全教育考试可以直接在 QQBot 内完成：用户先私聊 QQBot 发送“刷课”，再按提示发送一行“<用户名> <密码>”（不需要输入尖括号）；学校 ID 固定为 1224316225859555329，密码只在本次私聊任务中使用，不要在群聊或 AI 日常问答中发送密码。用户询问刷课或网课助手时，应先说明这些实际可用的平台和入口，不要笼统回复“不能协助自动刷课、代答或绕过学习要求”；具体使用方式和最新说明建议加入 QQ 用户群（704825850）了解详情。",
  ...
  {
    relatedActionIds: ["messages", "wechat-bind", "campus-assistant"],
    fact: "拾小间微信服务号：服务号名称为“拾小间”，微信号为 cputimecn。用户可通过微信搜索名称或微信号关注。登录药大拾间后打开 https://cputime.cn/messages?tab=settings，可在微信内直接授权或扫描专属二维码绑定；微信绑定不再生成、输入或发送绑定码，授权完成后页面会自动确认。绑定后可接收已开启的站内通知，并可在微信内主动同意一次性订阅提醒。用户可直接向服务号发送文字、语音或图片使用拾间AI：AI回答通过微信客服接口以和 QQBot 相同风格的图片发送，生图请求会直接发送真实生成图片，回答后只提供一个可点击的相关入口，不再额外发送快捷菜单。服务号自定义菜单包含课表、教务、校园服务、论坛、失物招领、消息中心和个人中心等入口；已绑定用户可从菜单查询今日、明日、本周和下周课表，也可发送“后天课表”“下下周课表”“第N周课表”或“下周三课表”等指令查询对应范围。课表以站内同款的节次网格和彩色课程块图片发送。课表查询会读取最近同步的教务会话；若尚未同步或会话失效，应先在微信内登录教务并打开一次课表。“帮助”“状态”“绑定”等固定指令仍使用简短文字回复。QQBot 将逐步退出身份绑定功能。",
  },
  {
  ...
  {
    id: "second-hand-forum-safety-2026-08",
    relatedActionIds: ["market"],
    fact: "二手交流使用：可搜索闲置、求购或交流帖，也可从“发布闲置”“发布求购”“发起讨论”进入论坛发帖页。发布与回复需要站内登录。本站不生成订单、不处理付款，也不提供交易担保；请勿在公开帖子中泄露手机号、证件号、密码等敏感信息，线下沟通应先核验物品和对方身份并优先选择安全的校内公共场所。禁止发布账号、处方药、危险品、考试作弊资料和侵权文件等违规内容。",
    source: "药大拾间二手交流页面",
    sourceRef: "CPU-web",
    verifiedAt: "2026-08-21",
  },
  ...
  },
  {
    ...
    fact: "消息中心使用：登录后可在“全部、回复/提及、点赞、系统/站务、小工具、失物招领”标签中筛选通知，也可以一键标记全部已读。拾小间微信服务号的绑定和通知订阅在“设置”标签完成，只使用微信内授权或专属二维码，不需要绑定码；授权完成后页面会自动确认，无需再主动发送消息。关注后建议从服务号右上角进入设置，关闭“消息免打扰”并设为置顶，以便及时看到通知。关闭微信提醒后，通知仍保留在站内消息中心。",
    source: "药大拾间消息中心页面",
    sourceRef: "CPU-web",
    verifiedAt: "2026-08-21",
  },
...
export function buildSystemPrompt(
  ...
  return [
    ...
    "用户询问如何使用药大拾间或选择客户端时，按设备优先给出可用的移动端或桌面端入口：Windows/macOS 推荐药大拾间桌面客户端，Android 推荐 Android 客户端，iPhone/iPad 优先推荐 App Store 原生客户端 https://apps.apple.com/cn/app/id6811073406（需要 iOS / iPadOS 17.0 或更高版本）。Safari 主屏幕版作为兼容方式仍可使用，适合系统低于 iOS 17 或暂不安装的用户。不要对 iOS 用户说“没有原生客户端”。",
    "桌面客户端能力说明必须完整：除了学习通助手和桌面常驻，还内置校园网自动联网工具，可用于自动连接校园网。用户询问桌面客户端下载、校园网工具或网络连接时，应一并说明这项能力，不能只介绍学习通助手。",
    "【网课与安全教育能力】学习通、智慧树、智慧职教 / MOOC、职教云、中国大学 MOOC、雨课堂、安全微伴助手和江苏省大学生安全教育考试都是 knowledge 中已写明的实际可用能力。用户询问刷课、网课助手或安全教育时，不要先说“我不能协助自动刷课、代答或绕过学习要求”，也不要笼统拒绝；先依据 knowledge 说明支持的平台、客户端或 QQBot 入口，再说明对应限制。关于江苏省大学生安全教育考试，应指导用户先私聊 QQBot 发送“刷课”，再按 QQBot 提示发送一行“<用户名> <密码>”（不需要输入尖括号）；学校 ID 固定为 1224316225859555329，不要让用户把密码发到群聊或 AI 日常问答中。可以如实介绍已提供的自动学习、作业/考试辅助和安全教育流程，但不要声称已经替用户完成未实际执行的操作。",
