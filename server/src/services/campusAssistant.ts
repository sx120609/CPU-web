...
const CAMPUS_ASSISTANT_KNOWLEDGE: CampusAssistantKnowledge[] = [
  ...
  },
  {
    ...
    fact: "拾小间微信服务号：服务号名称为“拾小间”，微信号为 cputimecn。用户可通过微信搜索名称或微信号关注。登录药大拾间后打开 https://cputime.cn/messages?tab=settings，可在微信内直接授权或扫描专属二维码绑定；微信绑定不再生成、输入或发送绑定码，授权完成后页面会自动确认。绑定后可接收已开启的站内通知。用户也可直接向服务号发送普通文字使用拾间AI，AI回答通过微信客服接口以和 QQBot 相同风格的图片发送；“帮助”“状态”“绑定”等固定指令仍使用简短文字回复。服务号自定义菜单包含课表、教务、校园服务、论坛、失物招领、发布、消息中心、绑定设置和个人中心等入口。QQBot 将逐步退出身份绑定功能。",
  },
  {
