...
const TRUSTED_CHECKOUT_HOSTS = new Set(["pay.kaipay.cn"]);
...
function resolveSafeCheckoutRedirect(value: string, submitUrl: URL) {
  try {
    ...
    const trustedHost = redirectUrl.hostname === submitUrl.hostname
      || TRUSTED_CHECKOUT_HOSTS.has(redirectUrl.hostname);
    return redirectUrl.protocol === "https:" && trustedHost ? redirectUrl.toString() : "";
  } catch {
    return "";
  }
}
...
export async function submitEpayCheckout(
  ...
  if (submitUrl.protocol !== "https:") {
    throw new Error("易支付网关必须使用 HTTPS");
  }
  ...
  try {
    ...
    if (response.status >= 300 && response.status < 400) {
      const redirectUrl = response.headers.get("location")
        ? resolveSafeCheckoutRedirect(response.headers.get("location")!, submitUrl)
        : "";
      return redirectUrl
        ? { ok: true, redirectUrl }
        : { ok: false, message: "支付平台没有返回可信的收银台地址", upstreamStatus: response.status };
    }
    ...
    if (response.ok && (contentType.includes("text/html") || /^\s*<!doctype\s+html|^\s*<html\b/i.test(body))) {
      ...
      return redirectUrl
        ? { ok: true, redirectUrl }
        : { ok: false, message: "支付平台返回了无法识别的收银台页面", upstreamStatus: response.status };
    }
    return {
      ok: false,
...
export async function updateEpayConfig(input: EpayConfigInput, requestOrigin = "") {
  ...
  if (input.gatewayUrl !== undefined) data.gatewayUrl = input.gatewayUrl.trim() ? normalizeAbsoluteUrl(input.gatewayUrl, "易支付网关必须使用 HTTPS") : "";
